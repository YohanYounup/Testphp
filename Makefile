...
clear-test-cache: ## Clear cache in test env
	$(DOCKER_COMPOSE_RUN) php php ./bin/console cache:clear --env=test
...
test-unit: clear-test-cache ## Launch unit tests
#	$(DOCKER_COMPOSE_RUN) php php ./vendor/bin/phpunit --testsuite Unit

test-functional: clear-test-cache ## Launch functional tests
	$(DOCKER_COMPOSE_RUN) php php ./vendor/bin/phpunit --testsuite Functional --stop-on-failure -vvv
